...
const TaskUpdate = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`/task/${id}`).then(res => {
      const t = res.data.task;
      setTask({
        title: t.title, description: t.description, project: t.project, 
        task_type: t.task_type || 'Task', labels: t.labels || '', sprint: t.sprint || '',
        assigned_to: t.assignee.id, priority: t.priority, status: t.status, 
        due_date: t.due_date.split('T')[0], estimated_hours: t.estimated_hours
      });
    });
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await axios.put(`/task/${id}`, { ...task, assigned_to: parseInt(task.assigned_to) });
  ...
  return (
    <div>
      <div className="mb-2">
        <Link to={`/task/${id}`} className="text-decoration-none text-muted" style={{ fontSize: '0.875rem' }}><i className="bi bi-arrow-left"></i> Back to Issue</Link>
      </div>
      
      <div className="jira-card p-4 mx-auto mt-2" style={{ maxWidth: '800px' }}>
        <h4 className="fw-bold mb-4" style={{ color: 'var(--jira-dark)' }}>Update Issue</h4>
        <form onSubmit={handleSubmit}>
          ...
          <div className="row mb-3">
            ...
            <div className="col-md-4">
              <label className="form-label fw-bold text-muted small mb-1">Status</label>
              <select className="form-select" style={{ borderRadius: '3px' }} value={task.status} onChange={e => setTask({...task, status: e.target.value})}>
          ...
          <div className="mb-4">
            <label className="form-label fw-bold text-muted small mb-1">New Attachment (Optional PDF)</label>
            <input type="file" className="form-control" accept=".pdf" style={{ borderRadius: '3px' }} onChange={e => setFile(e.target.files[0])} />
          </div>
          
          <div className="d-flex justify-content-end border-top pt-3">
            <button type="button" className="btn text-muted fw-bold me-2" onClick={() => navigate(`/task/${id}`)}>Cancel</button>
            <button type="submit" className="btn-jira-primary px-4">Update Issue</button>
          </div>
        </form>
      </div>
    </div>
